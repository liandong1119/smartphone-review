<template>
  <div class="recommend-reviews-container">
    <div class="page-header">
      <h1 class="page-title">推荐评测</h1>
      <div class="page-description">根据您的兴趣为您推荐相关评测</div>
    </div>
    ...
    <!-- 空数据状态 -->
    <el-empty v-else-if="posts.length === 0" description="暂无推荐评测" />
...
// 获取推荐评测
const fetchRecommendReviews = async () => {
  loading.value = true
  try {
    const response = await postApi.getRecommendPosts({
      page: currentPage.value,
      limit: pageSize.value
    })
    console.log('获取推荐评测响应:', response)
    ...
    if (response) {
      ...
      ElMessage.error('获取推荐评测失败')
    }
  } catch (error) {
    console.error('获取推荐评测失败:', error)
    ElMessage.error('获取推荐评测失败')
  } finally {
    loading.value = false
  }
