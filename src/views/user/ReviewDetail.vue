    ...
    <div class="review-detail-container">
        <div class="review-detail-card" v-if="reviewDetail">
            ...
            <div class="detailed-rating-section" v-if="reviewDetail.rating">
                ...
                <div class="rating-breakdown" v-if="hasDetailedRatings">
                    <div class="breakdown-row" v-if="reviewDetail.appearanceRating">
                        ...
                        <div class="breakdown-progress">
                            <div class="breakdown-bar"
                                 :style="{width: `${(reviewDetail.appearanceRating/5)*100}%`}"></div>
                        </div>
                    ...
                    <div class="breakdown-row" v-if="reviewDetail.performanceRating">
                        ...
                        <div class="breakdown-progress">
                            <div class="breakdown-bar"
                                 :style="{width: `${(reviewDetail.performanceRating/5)*100}%`}"></div>
                        </div>
            ...
            <div class="phone-tags">
                ...
                <span class="phone-model-tag"
                      v-if="!reviewDetail.phoneModel && reviewDetail.model">{{ reviewDetail.model }}</span>
            </div>
...
const replayToCommentId = ref(null) // 要回复的评论的id
...
const addReply = async () => {
    ...
    try {
        ...
        const result = await commentApi.createComment(reviewId.value, {
            ...
            parentId: currentReplyToId.value,
        })
...
const invokeLikeComment = async (comment) => {
    if (comment.isLiked) {
        // 取消点赞
...
const replyToReply = (comment, reply, commentIndex) => {
    ...
    console.log("当前要回复的评论id为；",currentReplyToId.value)
}

// 点赞回复
const toggleReplyLike =  (reply) => {
    invokeLikeComment(reply)
    ...
    // if (reply.isLiked) {
    //     // 取消点赞
    //     await commentApi.unlikeComment(reviewId.value, reply.id)
    //     reply.isLiked = false
    //     if (reply.likes > 0) {
    //         reply.likes--
    //     }
